...
export function SmartImage({
  ...
  // Generate URLs
  const finalSrc = unsplashId
    ? getOptimizedImageUrl(unsplashId, { width, height, fit, quality })
    : src || "";
  ...
  return (
    <motion.div
      key={finalSrc}
      className={cn(
        "relative overflow-hidden bg-[var(--bg-card)] border border-white/5",
        wrapperClassName
      )}
    ...
    >
      {/* Skeleton Pulse during first load */}
      {!loaded && !error && (
