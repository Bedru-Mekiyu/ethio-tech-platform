    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {Array.from({ length: 6 }).map((_, i) => (
          <Card key={i} className="border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              <div className="h-12 w-12 rounded-full bg-slate-100 skeleton-shimmer" />
            ...
            <div className="space-y-2">
              <div className="h-3 bg-slate-100 rounded skeleton-shimmer" />
    ...
    <div className="space-y-6">
      ...
      {filtered.length ? (
              ...
              <Card className="h-full border-slate-200 bg-white p-5 shadow-sm transition-all duration-200 hover:border-zinc-300 hover:shadow-md">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-sm font-semibold text-slate-900 truncate">{mentor.fullName}</h3>
                    {mentor.rating != null ? (
                ...
                {mentor.bio ? <p className="text-xs text-slate-600 line-clamp-2 mb-3">{mentor.bio}</p> : null}
                {mentor.expertise && mentor.expertise.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {mentor.expertise.slice(0, 3).map((exp) => (
                      <span
                        key={exp}
                        className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200 text-[11px] font-medium text-slate-700"
                      >
                        {exp}
                      </span>
                    ))}
                    {mentor.expertise.length > 3 ? (
                      <span className="px-2 py-0.5 rounded-md bg-slate-50 border border-slate-200 text-[11px] font-medium text-slate-500">
                        +{mentor.expertise.length - 3}
                      </span>
                    ) : null}
                  </div>
        ...
        <Card className="border-slate-200 bg-white p-8 text-center shadow-sm">
          <p className="text-sm text-slate-500">No mentors found matching your criteria.</p>
        </Card>
      )}
    </div>
  );
