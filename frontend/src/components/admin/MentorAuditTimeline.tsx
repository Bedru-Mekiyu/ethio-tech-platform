...
function AuditItem({ log }: { log: AuditLogEntry }) {
  return (
    <div className="relative border-l border-zinc-200 pl-4 pb-4 last:pb-0">
      <div className="absolute -left-1.5 top-1 h-3 w-3 rounded-full border-2 border-zinc-900 bg-white" />
      <p className="text-sm font-semibold capitalize text-zinc-900">{formatAction(log.action)}</p>
      <p className="text-xs text-zinc-500">
        {log.actor?.fullName ?? "System"} · {new Date(log.createdAt).toLocaleString()}
      </p>
      {log.metadata && Object.keys(log.metadata).length > 0 && (
        <p className="mt-1 text-xs text-zinc-600 font-mono">{JSON.stringify(log.metadata)}</p>
      )}
    </div>
  );
}
